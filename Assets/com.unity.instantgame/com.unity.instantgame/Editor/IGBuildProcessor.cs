...
namespace Unity.InstantGame
{
    ...
    /**
    class IGUnityLinkerProcessor : IUnityLinkerProcessor
    {
        ...
        public string GenerateAdditionalLinkXmlFile(BuildReport report, UnityEditor.UnityLinker.UnityLinkerBuildPipelineData data)
        {
            ...
            if (PlayerSettings.GetScriptingBackend(BuildTargetGroup.Android) == ScriptingImplementation.Mono2x)
            {
                ...
                foreach (var module in IGBuildPostprocessor.dllFilesInEngines)
                {
                    sb.AppendLine($"\t<assembly fullname=\"{module}\" preserve = \"all\"/>");
                }
                sb.AppendLine("</linker>");
            }

            if (!Directory.Exists(IGBuildConstants.k_OutputDir))
                Directory.CreateDirectory(IGBuildConstants.k_OutputDir);

            var path = Path.Combine(IGBuildConstants.k_OutputDir, "IG_Module_Preserve.xml");
    ...
    **/
    ...
    class IGBuildPostprocessor : IPostprocessBuildWithReport
    {
        ...
        /**
        private static readonly string[] k_DllFilesInEngines =
        {
            "Mono.Security",
            "mscorlib",
        ...
        **/
        internal static List<string> dllFilesInEngines
        {
            get
            {
                /**
                List<string> preserveModules = new List<string>(k_DllFilesInEngines);
                var targetGroup = BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget);
                if (PlayerSettings.GetApiCompatibilityLevel(targetGroup) == ApiCompatibilityLevel.NET_Standard_2_0)
                {
                    preserveModules.Add("netstandard"); //only include in .net standard
                }
        ...
        private static void GenerateIGAndroidOutFile()
        {
            ...
            igUnity.iconUrl = ASCCDHelper.CCDInstanceId + "/icon.png";   //default name:  icon.png
            ...
            switch (ASCCDHelper.TargetPlatform)
            {
                case "Kwai":
        ...
        private static void PrepareEngineDir(string abiStr)
        {
            ...
            string managedDir = k_GameMainDir + "/assets/bin/Data/Managed/";
            if (!Directory.Exists(managedDir))
                return;

            Directory.CreateDirectory(engineDir + "/Managed");
            foreach (var dllFile in dllFilesInEngines)
            {
                if (File.Exists(managedDir + dllFile + ".dll"))
                {
                    File.Copy(managedDir + dllFile + ".dll", engineDir + "/Managed/" + dllFile + ".dll");
                    if (File.Exists(managedDir + dllFile + ".pdb"))
                    {
                        File.Copy(managedDir + dllFile + ".pdb", engineDir + "/Managed/" + dllFile + ".pdb");
                    }
                }
            }
        }
        ...
        private static void PrepareGameMainDir()
        {
            ...
            foreach (var dllFile in dllFilesInEngines)
            {
                ...
                File.Delete(managedDir + dllFile + ".dll");
                if (File.Exists(managedDir + dllFile + ".pdb"))
                {
                    File.Delete(managedDir + dllFile + ".pdb");
                }
            }
        ...
        private static void GenerateBundleIfNeeded(string bundleName, List<IGBundle> bundles, IGBundleType type, string abiStr = "")
        {
            ...
            bundle.download_link = ASCCDHelper.CCDInstanceId + "/" + bundle.filename;
